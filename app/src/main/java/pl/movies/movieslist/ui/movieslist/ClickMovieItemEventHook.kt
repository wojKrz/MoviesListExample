...
class ClickMovieItemEventHook(
    private val onMovieClickListener: (Long) -> Unit
    ...
    override fun onBind(viewHolder: RecyclerView.ViewHolder): View? =
        if (viewHolder is BindingViewHolder<*> && viewHolder.binding is ItemNowPlayingMovieBinding)
            (viewHolder.binding as ItemNowPlayingMovieBinding).rootView
        else
            null
    ...
    override fun onClick(
        ...
        onMovieClickListener(item.model.id)
    }
}
